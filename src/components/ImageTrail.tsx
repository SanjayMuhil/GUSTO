...
class ImageTrailVariant1 {
  ...
  targetEventElement: HTMLElement;
  DOM: { el: HTMLElement };
  images: ImageItem[];
  imagesTotal: number;
  imgPosition: number;
  ...
  constructor(container: HTMLElement) {
    ...
    this.targetEventElement = container.parentElement || container;
    ...
    this.targetEventElement.addEventListener('mousemove', this.handlePointerMove);
    this.targetEventElement.addEventListener('touchmove', this.handlePointerMove);
    ...
    this.initRender = (ev: any) => {
      ...
      this.targetEventElement.removeEventListener('mousemove', this.initRender);
      this.targetEventElement.removeEventListener('touchmove', this.initRender);
    };
    this.targetEventElement.addEventListener('mousemove', this.initRender);
    this.targetEventElement.addEventListener('touchmove', this.initRender);
  }
  ...
  destroy() {
    ...
    this.targetEventElement.removeEventListener('mousemove', this.handlePointerMove);
    this.targetEventElement.removeEventListener('touchmove', this.handlePointerMove);
    this.targetEventElement.removeEventListener('mousemove', this.initRender);
    this.targetEventElement.removeEventListener('touchmove', this.initRender);
...
class ImageTrailVariant6 {
  ...
  mapSpeedToSize(speed: number, minSize: number, maxSize: number): number {
    const maxSpeed = 200;
    return minSize + (maxSize - minSize) * Math.min(speed / maxSpeed, 1);
  }
  mapSpeedToBrightness(speed: number, minB: number, maxB: number): number {
    const maxSpeed = 70;
    return minB + (maxB - minB) * Math.min(speed / maxSpeed, 1);
  }
  mapSpeedToBlur(speed: number, minBlur: number, maxBlur: number): number {
    const maxSpeed = 90;
    return minBlur + (maxBlur - minBlur) * Math.min(speed / maxSpeed, 1);
  }
  mapSpeedToGrayscale(speed: number, minG: number, maxG: number): number {
    const maxSpeed = 90;
...
class ImageTrailVariant8 {
  ...
  cachedRotation: { x: number; y: number };
  zValue: number;
  cachedZValue: number;
  handlePointerMove: (ev: any) => void;
  initRender: (ev: any) => void;
