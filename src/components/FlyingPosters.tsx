...
function AutoBind(self: any, { include, exclude }: AutoBindOptions = {}) {
  const getAllProperties = (object: any): [any, string][] => {
    const properties: [any, string][] = [];
    do {
      for (const key of Reflect.ownKeys(object)) {
        properties.push([object, key as string]);
      }
...
class Media {
  ...
  onResize(opts: { screen?: any; viewport?: any } = {}) {
    const { screen, viewport } = opts;
...
class Canvas {
  ...
  constructor({
    ...
    try {
      ...
      console.error("FlyingPosters initialization failed:", error);
    }
  }
...
export default function FlyingPosters({
  ...
  useEffect(() => {
    ...
    const gl = canvasEl.getContext("webgl2", { alpha: true, antialias: true });
    if (!gl) {
      console.warn("FlyingPosters: WebGL2 not supported");
      return;
    }
